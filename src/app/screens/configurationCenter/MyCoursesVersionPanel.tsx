...
export function MyCoursesVersionPanel({ ctx }: { ctx: MyCoursesBuilderViewContext }) {
  const {
        ...
        <div className="max-w-2xl space-y-5">
          ...
          <div className="bg-white rounded-xl border p-5" style={{ borderColor: P.border }}>
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p
                  ...
                  style={{ fontFamily: "'Plus Jakarta Sans',sans-serif", color: P.olive }}
                >
                  v2.1
                </p>
              </div>
            ...
            <div className="space-y-2">
              {VERSIONS.map(({ version, status, date, current, notes }) => (
                <div
                  key={version}
                  className="flex items-center gap-4 p-3 rounded-xl"
                  style={{
                    background: current ? P.lightSage : P.bg,
                    border: `1px solid ${current ? P.olive + "50" : P.border}`,
                  }}
                >
                  <div className="flex items-center gap-2 w-16">
                    <GitBranch size={13} style={{ color: current ? P.olive : P.textMuted }} />
                    <p
                      className="text-sm font-bold font-mono"
                      style={{ color: current ? P.olive : P.text }}
                    >
                      {version}
                    </p>
                  </div>
                  <Chip
                    label={status}
                    variant={
                      status === "Published" ? "green" : status === "Archived" ? "neutral" : "sage"
                    }
                  />
                  {current && <Chip label="Current" variant="sage" />}
                  <p className="text-xs flex-1" style={{ color: P.textMuted }}>
                    {notes}
                  </p>
                  <p className="text-[10px] font-mono flex-shrink-0" style={{ color: P.textMuted }}>
                    {date}
                  </p>
          ...
          <AICard title="Version History Note">
            <p className="text-xs leading-relaxed" style={{ color: "#7A5A10" }}>
              Learners who completed v1.x will retain their completion records. Historical learning
              data is preserved across all versions.
            </p>
          </AICard>
        </div>
      )}
